...
//NOTE: [todo] fix this bug: find a way to fix the path to the current dir in windows.
#ifndef WIN32
 system("pwd > peps_tmp");
...
bool Provide_DSC()
{
 ...
 peps_name cnd_level_name; // directory to store the internal descriptor   (./dsc/*.dsc)
 int      go_on  = 1;
                       ...
                       const func_id f, const ev_id e, const aut_id a)
{
 ...
 ///rp        min;
 ///rp        max;               // minimum and maximum evaluation of the function
 aut_set   to_know;                // the set of automata to be specified by the user
 aut_st    sizes;                  // the size of each automaton
 peps_name buf1, buf2, buf3;        // buffers to names from dictionary
 ///peps_name buf4;
...
m_typ Ask_a_Product_Method()
{
 ...
 cout << "Choose a multiplication method:\n"
      ...
      << "    5 to used a mixed method\n";
 cin >> i;
 switch (i) {
   case  1 : answer = meth_A; break;
   case  2 : answer = meth_B; break;
   case  3 : answer = meth_C; break;
   case  4 : answer = meth_S;
             PRF::prf.strategy(Ask_a_Strategy_Method());
             break;
...
//====================================================================================//
//  Ask a Strategy to 'cut' tensor terms                                              //
//====================================================================================//
strat_typ Ask_a_Strategy_Method()
{
 int   i;
 strat_typ answer;
 cout << "Choose a strategy to 'cut' tensor terms:\n"
 ...
 cin >> i;
 switch (i) {
   case  1 : answer = strat_A; break;
   case  2 : answer = strat_B; break;
   case  3 : answer = strat_C; break;
   case  4 : answer = strat_D; break;
   case  5 : answer = strat_E; break;
   default : answer = strat_A;
             cout << "Assuming identities to shuffle (strategy A)\n";
 }
 return(answer);
}
//====================================================================================//
                      ...
                      const rp resid)
{
 ...
 out_file << "\nThe multiplication method ";
   ...
   case meth_S : out_file << " use a Split approach (method S)"; break;
   case meth_M : out_file << " can be different for each term (mixed method)"; break;
   }
 if (PRF::prf.method() == meth_S) {
   out_file << "\nThe strategy to 'cut' the tensor terms:\n";
   switch (PRF::prf.strategy()) {
    case strat_A : out_file << "   identities to shuffle [constant] (strategy A)"; break;
    case strat_B : out_file << "   functional evaluations in shuffle [functional] (strategy B)"; break;
    case strat_C : out_file << "   functional evaluations in split [functional] (strategy C)"; break;
    case strat_D : out_file << "   functional evaluations in split, only identities to shuffle [functional] (strategy D)"; break;
    case strat_E : out_file << "   only functional definition and identities in shuffle [functional] (strategy E)"; break;
   }
 }
 ...
 if (PRF::prf.method() == meth_S) {
   out_file << "\nNumber of AUNFs: " << CND::cnd.get_tot_aunfs();
   double mem_aunf = CND::cnd.get_mem_aunfs() / 1024;
   out_file << " (" << mem_aunf << " Kb)\nChosen cuts: [ ";
   for (int e = 0; e < CND::cnd.ev(); e++) {
       out_file << CND::cnd.get_fcut(e) << " ";
   }
   out_file << "]" << endl;
   out_file << "Synchr. Term Information: evt. [ automata permutation orders and cut (|) information ] [ identities list]" << endl;
   for (int e = 0; e < CND::cnd.ev(); e++) {
       aut_list l = CND::cnd.get_s_B_order(e);
       int cut = CND::cnd.get_fcut(e);
       //PERMUTATION ORDER INFORMATION
       out_file <<setw(5)<< e << ". [";
       if (cut == 0)
           out_file << "|";
       else
       ...
       for (int a = 0; a < l.size(); a++) {
           out_file <<setw(2)<< l.aut(a) << (a == cut-1 ? "|" : (cut==l.size()+1?"":" "));
       }
       if (cut == l.size()+1) 
           out_file << "|";
       out_file << "]";// << endl;
       ...
       if (cut == 0)
           ...
           out_file << " ";
       for (int a = 0; a < l.size(); a++) {
           bool type = (CND::cnd.get_sf_type(e, l.aut(a)) == idnt_mat);
           out_file << type << (a == cut-1 ? "|" : (cut==l.size()+1?"":" "));
       }
       if (cut == l.size()+1) 
           out_file << "|";
   ...
   out_file << "Time needed to generate the AUNFs list: " 
            << rp(int(CND::cnd.get_aunf_time()*100))/100 << " seconds" << endl;
 }
...
{
 peps_name cnd_level_name, descriptor_name, model_name, f_name;
 ///peps_name v_name;
 ofstream  out_file;
 ///v_impl  vv;
...
void Show_Credits()
{
 cout << "\n\n     GTAexpress - Kronecker Descriptor Solver"
      << "\n\n     This version of GTAexpress was released on: " << __RELEASE_DATE__
...
peps_options Welcome_Get_Peps_Option()
{
 ...
 do
{cout << "\n\n     +--------------------------------------------------------+"
      <<   "\n     |    This is GTAexpress - Kronecker Descriptor Solver    |"
      <<   "\n     |  released on: " << __RELEASE_DATE__  << " -- compiled on: " << __DATE__ << "  |"
#ifdef _PEPS_DEBUG_
      <<   "\n     |                  *** debug version ***                 |"
#endif
      <<   "\n     +--------------------------------------------------------+"
      << "\n\n  1) Compile a model                 5) Load/Show/Remove data structures"
      <<   "\n  2) Solve a compiled model          6) Inspect data structures"
      <<   "\n  3) Probability vector facilities   7) Sparse matrix facilities"
      <<   "\n  4) Preferences                     8) About this version"
      << "\n\n     0) Exit (Option 0 always exits the current menu)\n";
 ...
 default : cout << "\n\n            ******* Before Leaving *******"
                ...
                <<   "\n 0) Don't leave, i.e., return to main menu\n";
           answer = goodbye;
           cin >> option;
           switch (option) {
           case '1': break;
