...
//=======================================================================================//
void Compile_SAN_2007(bool aggr_too, bool lump_too, bool norm_too)//Compile SAN for PEPS 2007 //
//=======================================================================================//
{
  ifstream  f_tmp;
  char    filename[max_peps_name];
  char    filename_full[max_peps_name];
  ...
  strcpy(filename, san_level_name);  
  sprintf(filename_full,"%s.san",filename);
  g.compile(filename_full);

  strcpy(des_level_name, "des/");  strcat(des_level_name, filename);
  strcpy(dsc_level_name, "dsc/");  strcat(dsc_level_name, filename);
  strcpy(agg_level_name, "agg/");
   
  f_tmp.open("des/peps_des");
  if (f_tmp) {
   f_tmp.close();
  }
  else{
   cout << "Creating description files directory\n";
   system("rm -rf des");
   system("mkdir des");
   system("pwd > des/peps_des");
  }
  cout << "Creating description files model\n";
  sprintf(filename_full,"des/%s.des",filename);
  g.put2file_des(filename_full);
  sprintf(filename_full,"des/%s.dic",filename);
  g.put2file_dic(filename_full);
  sprintf(filename_full,"des/%s.fct",filename);
  g.put2file_fct(filename_full);
  sprintf(filename_full,"des/%s.tft",filename);
  g.put2file_tft(filename_full);
  sprintf(filename_full,"des/%s.res",filename);
  g.put2file_res(filename_full);
  cout << "The description files to " << filename << " model were created in \"des\" directory." << endl;
   ...
   if (Permission_to_Proceed(dsc_level_name, dsc_file)) {            // .des compilation
     ...
     if (aggr_too || lump_too) {                                     // aggregation or lump
       ...
       Ask_a_File_Name(filename, dsc_file);
       strcat(agg_level_name, filename);
       strcat(cnd_level_name, filename);
...
//====================================================================================//
   ...
   case comp_2007       : Compile_SAN_2007(false, false, false); break;
...
//                       break;
   ...
   case power_diag   : if (Start_Up_CND(buf))      Uniformization_CND(buf, 
